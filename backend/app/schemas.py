...
class ProcessoBase(BaseModel):
    ...
    numero_da_guia: Optional[str] = None
...
class ProcessoComEventos(Processo):
    ...
    )  # Um processo terá uma lista de seus eventos


class PresoDetalhe(Preso):
    processos: List[ProcessoComEventos] = Field(
        default_factory=list
    )  # Um preso terá uma lista de seus processos (com eventos)
...
class ProcessoConsultaIntegracaoRequest(BaseModel):
    ...
    fontes: List[Literal["datajud", "pje"]] = Field(
        default_factory=lambda: ["datajud", "pje"]
    )
